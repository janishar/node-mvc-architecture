...
class User extends Model {
    ...
    create(accessTokenKey) {

        return Query.transaction(connection => {

            return super.createInTx(connection)
                .then(user => {
                    let userAccess = new UserAccess(user._id, accessTokenKey);
    ...
    update(accessTokenKey) {
        ...
        return Query.transaction(connection => {
            ...
            return super.updateInTx(connection, new QueryMap().put('id', this._id))
                .then(user => {
                    let userAccess = new UserAccess(user._id, accessTokenKey);
                    return userAccess.updateInTx(connection, new QueryMap().put('user_id', this._id))
                })
                .then(useraccess => {
                    return Promise.resolve(this);
                })
