...
let FailureRedirectResponse = require('./../helpers/response').FailureRedirectResponse;
...
router.use(
    (req, res, next) => {
        ...
        req.checkCookies('access-token', 'Access token is empty').notEmpty();
        req.checkCookies('user-id', "User id is empty").notEmpty();
        req.checkCookies('user-id', "User id is invalid").isInt();

        const validErr = req.validationErrors();

        if (validErr) {
            return new FailureRedirectResponse().send(res);
        }

        req.userId = req.cookies['user-id'];
        ...
        new UserAccess().getFromUser(req.userId)
            ...
            .then(payLoad => {
                ...
                if (!payLoad
                    ...
                    throw new AccessTokenError("Invalid Access Token").send(res)
                }
